...
```
 ...
 Before abandoning this dataset, I will also use it to show how the number of rows is different when the default option `end="max"` is used. This is because one line is completely dropped as it goes extinct in all replicates. Hence my remark before of not using the default option (and why Pext will throw an error if detects different number of rows in the collated data)!
