...
changes). Once I am happy with these scenarios, I run lots of replicates for each of them and then evaluate how these changes have affected the population trajectories and/or other population parameters (e.g. genetic distance). Depending on what I am interested into, I may
'evaluate' these changes by conducting pairwise comparisons. This means that I need some tools to collate results, calculate descriptive statistics across a number of replicates, and conduct statistical tests for the pairs of scenarios of my interest. If you are working on a project with a similar approach, hopefully, HexSimR may be useful to you too. If you have a different set up between different scenarios but you ran several replicates, HexSimR may be still be valuable to process the data within each scenario.
...
where $V_{i}$ is the mean value of the variable of interest in the $i$ scenario being compared, $V_{B}$ is the mean value in the baseline scenario, and $s_i$ and $s_B$ are their respective standard deviation. 
...
```Pext``` calculates the probability of extinction starting from the census files and you can select any trait that it is included in the census event you indicated with the argument ```headers```. This makes this function quite powerful as it can be used to calculate the probability of any trait(s) you managed to include in a census file to be zero (or non-zero by calculating 1 - Pext).  
...
You may have already noted that SSMD can't be computed if both $s$ are zero. This is likely to happen when the populations always go extinct, or never go extinct as it is in this example. In this case, the cells in the returned xls file are blank. On the other extreme, I'm sure you have alredy noted that that if the mean parameters are the same, then SSMD=0 and p-value=0.5.  
...
* The headings of the coulumns we want to include in the table (as a ```list```, one element for each file to be processed).  
...
Then I add to the same xls file (Tables.xlsx) a sheet with the name "Ranges_descrip", where I summarise the descriptive statistics obtained from the ranges report. Note that, by default, the ```colh``` argument is a subset of the headings of the ranges report, which are the ones I'm interested into, so I don't include this argument here and use the default.   
...
As an example, I also add to the table the SSMD results for the census data:
```{r}
...
That's it, you made it to the end of this tutorial! I hope it helped to understand some of HexSimR's functionalities and, while I can't prommise any prompt reply, I will be grateful for any feedback, positive and negative.
