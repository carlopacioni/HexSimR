...
The following lines in the code below calculate the sex ratios inside and outside the fence. I included a census event stratified by age and sex in the model. I won't explain what each trait is (I'm sure you trust me on this), but the basic idea is that I first sum together all males inside and outside, respectively. The all females inside the fence,  and lastly I calcualte the ratios.  Once all these calculations are done, with the function `collate.census` HexSimR calculates the mean and SD across all 200 iterations. Note, that `collate.census` does this for all census files that finds, so if I don't have to specified the census files i want to include, I'll obtain the average for each of them in one go regardless as to whether I have performed any calculation on them.
...
* The subset of time steps to include in the table (or leave NULL if want to keep all)
...
table <- make.table(path.results, 
                    ...
                    colh=list(c("SexRatio_In", "SexRatio_Out"),
                              c("Inside", "Outside")),
                    vround=1, sdround=3, ts=NULL, table.name="Tables.xlsx", 
...
Then I add to the same xls file (Tables.xlsx) a sheet with the name "Ranges_descrip", where I summarise the descriptive statistics obtained from the ranges' report. Note that, by default, the ```colh``` argument are a subset of the headings of the ranges report, which are the ones I'm interested into, so I don't include this argument here and leave the default.   
...
I had glided over the secelction of the events when I called ```m.range.rep``` before, but by selecting these two events I actually have data pre- and post- dispersal. Because I'm the only one who knows this at the moment, I'll make it a bit more explicit by adding a column to the table (note that the object returned by ```make.table``` is a ```data.table``` so I load this package before to edit, but if you are not familiar with ```data.table``` you can work with them as they were normal ```data.frame``` in R), re-ordering the column order and then use ```knitr::kable``` to print it in a more readable format in this document:  
...
But, If you only want to save the updated table, you can do that with the following:
```{r}
...
table_SSMD_dem <- make.table(path.results, 
                         ...
                         colh=list(c("SexRatio_In", "SexRatio_Out"),
                                   c("Inside", "Outside")),
                         vround=3, sdround=3, ts=NULL, table.name="Tables.xlsx", 
...
result file. When ```SSMD=TRUE```, you have to pass a character vector to select the scanrios you want to include in your table. If you don't want to type in the names of all scenarios, you can obtain them with;
```{r}
