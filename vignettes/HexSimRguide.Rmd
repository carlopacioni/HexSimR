...
To find out where the example data are we run the following, which loads the package and store the location of the example data in `example_dir`.
...
Then, we create a temporary directory where we can copy the files (and sub-directories) so that we don't mess up with your HexSimR installation:
```{r}
duplicate <- tempdir()
...
remove the legend because, for example, you already have these information in the 
...
each senario processed. Each of these elements is also a list with three elemnts:
the first is the mean for each year.
```{r}
# The follwoing prints the mean Group size and Resources for each year of the 
# first scenario
m.range.rep[[1]][[1]][, 1:4, with=FALSE]
```

The second and the third are, respectively,
the mean and the standdard deviation across years between `start` and `end` for 
each `events`. The events are the events you inserted in HexSim sequence. If you
leave the default `NULL`, then all events are considered (see the help file for 
...
Firstly, you have to create an array of hexagons. As mentioned before, I have created a linear array (__Figure 2__).  
...
__RUN THIS CODE CHUNK AFTER GENETIC DATA ARE INSERTED__
```{r, eval=FALSE}
